 ...
 * @property Form $form
 */
class SimulationFormBackup extends SimplePage
{
    ...
    protected array $banks = [
        ...
        'itau-agencia' => [
            'name' => 'Itaú Agência',
            'logo' => 'https://www.itau.com.br/assets/dam/publisher/01_itau/42_landing_pages/new_home/icones-apps/sup_app_car_img_92x92.png'
        ],
        'bradesco-exclusive' => [
    ...
    protected function getFormSchema(): array
    {
        return [
            ...
            Grid::make('location')
                ->columns()
                ->schema([
                    Select::make('state_uf')->label('Estado')
                        ->searchable()
                        ->preload()
                        ->options(
            ...
            Radio::make('property_state')
                ->columns()
                ->label('Condições do Imovel')
                ->options(PropertyStateEnum::class)
                ->required(),

            Radio::make('live_work_in_city')
                ->columns()
                ->label('Reside ou Trabalha na Cidade Informada?')
                ->boolean()
                ->required(),

            Radio::make('property_or_loan_in_city')
                ->columns()
                ->label('Possui Imóvel ou Financiamento na Cidade?')
                ->boolean()
                ->required(),

            Radio::make('has_fgts')
                ->columns()
                ->label('Possui conta do FGTS com 36 ou mais contribuições?')
                ->boolean()
                ->required(),

            Radio::make('has_fgts_with_percent_of_value')
                ->columns()
                ->label('Possui conta ativa ou saldo no FGTS superior a 10% do valor do imóvel?')
                ->boolean()
                ->required(),
    ...
    public function generateResults(): void
    {
        ...
        $data['document'] = $this->customerDocument;
